...
#define DT_GNU_HASH 0x6ffffef5
    ...
    std::unordered_map<std::string, uintptr_t> ElfScanner::getSymbolsMap() const
    {
        ...
        const auto *symbol_table = reinterpret_cast<const uint8_t *>(_symbolTable);
        const auto *string_table = reinterpret_cast<const char *>(_stringTable);

        if (_elfHashTable)
        {
            const auto *elf_hash = reinterpret_cast<const uint32_t *>(_elfHashTable);
            const size_t num_chain = elf_hash[1];

            for (size_t i = 0; i < num_chain; i++)
            {
                const auto *symbol = reinterpret_cast<const KT_ElfW(Sym) *>(symbol_table + (_syment * i));
                if (!symbol || symbol->st_name >= _strsz || !symbol->st_value)
                    continue;

                std::string sym_str = std::string(string_table + symbol->st_name);
                if (!sym_str.empty())
                {
                    symbols[sym_str] = get_sym_address(symbol);
                }
            }
        }
        else if (_gnuHashTable)
        {
            const auto *gnu_hash = reinterpret_cast<const uint32_t *>(_gnuHashTable);
            ...
            for (uint32_t bucket_idx = 0; bucket_idx < num_buckets; bucket_idx++)
            {
                ...
                while (true)
                {
                    const auto *symbol = reinterpret_cast<const KT_ElfW(Sym) *>(symbol_table + (_syment * sym_idx));
                    if (!symbol || symbol->st_name >= _strsz)
                        break;
